...
import { getAwardedProposals } from '../../../api/adminApi';

export default function AwardedProposals() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await getAwardedProposals();
        setProposals(data);
      } catch (err) {
  ...
  return (
    <DashboardLayout role={user?.role}>
      <PageHeader
        title="Awarded Proposals"
        subtitle={`${proposals.length} proposal${proposals.length !== 1 ? 's' : ''} successfully funded`}
      />
      {error && <Alert variant="danger">{error}</Alert>}
      <Card>
        <Table columns={columns} data={proposals} emptyMessage="No awarded proposals found." />
      </Card>
    </DashboardLayout>
  );
}